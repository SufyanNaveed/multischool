...
class Category extends MY_Controller {
    ...
    public function index($school_id = null) {
        ...
        $this->data['category'] = $this->category->get_category_list($school_id);  
        
        $condition = array();
        $condition['status'] = 1;        
        if($this->session->userdata('role_id') != SUPER_ADMIN){            
            $condition['school_id'] = $this->session->userdata('school_id');        
            $this->data['category'] = $this->category->get_list('category', $condition, '','', '', 'id', 'ASC');
        }   
    ...
    public function add() {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                $insert_id = $this->category->insert('category', $data);
                if ($insert_id) {                    
                    create_log('Has been created a category :'.$data['name']);
                    
                    success($this->lang->line('insert_success'));
                    redirect('procurement/category/index/'.$data['school_id']);
                } else {
                    error($this->lang->line('insert_failed'));
                    redirect('procurement/category/add');
                }
            } else {
                error($this->lang->line('insert_failed'));
        ...
        $this->data['category'] = $this->category->get_category_list();      
        
        $condition = array();
        $condition['status'] = 1;        
        if($this->session->userdata('role_id') != SUPER_ADMIN){            
            $condition['school_id'] = $this->session->userdata('school_id');        
            $this->data['teachers'] = $this->category->get_list('category', $condition, '','', '', 'id', 'ASC');
        }        
        $this->data['schools'] = $this->schools;
    ...
    public function edit($id = null) {       
        ...
        if(!is_numeric($id)){
           ...
           redirect('procurement/category/index');  
        }
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                $data = $this->_get_posted_class_data();
                $updated = $this->category->update('category', $data, array('id' => $this->input->post('id')));
                ...
                if ($updated) {
                    ...
                    redirect('procurement/category/edit/' . $this->input->post('id'));
                }
            } else {
                 error($this->lang->line('update_failed'));
                $this->data['category'] = $this->category->get_single('category', array('id' => $this->input->post('id')));
            }
        }
        
        if ($id) {
            $this->data['category'] = $this->category->get_single('category', array('id' => $id));
   ...
   {             
      ...
      {   
          $name = $this->category->duplicate_check($this->input->post('school_id'), $this->input->post('name')); 
          if($name){
         ...
         $name = $this->category->duplicate_check($this->input->post('school_id'), $this->input->post('name'), $this->input->post('id')); 
          if($name){
                $this->form_validation->set_message('name', $this->lang->line('already_exist'));         
                return FALSE;
          } else {
    ...
    public function delete($id = null) {
        ...
        $category = $this->category->get_single('category', array('id' => $id));
        
        if ($this->category->delete('category', array('id' => $id))) {
        ...
        redirect('procurement/category/index/'.$category->school_id);
    }
    ...
    private function __create_default_section($insert_id){
        ...
        $data['created_by'] = logged_in_user_id();
        $data['status']     = 1; 
        $this->category->insert('sections', $data);
    }
