...
class Department extends MY_Controller {
    ...
    public function index($school_id = null) {
        ...
        $this->data['department'] = $this->department->get_department_list($school_id);  
        
        $condition = array();
        $condition['status'] = 1;        
        if($this->session->userdata('role_id') != SUPER_ADMIN){            
            $condition['school_id'] = $this->session->userdata('school_id');        
            $this->data['department'] = $this->department->get_list('department', $condition, '','', '', 'id', 'ASC');
        }   
    ...
    public function add() {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                $insert_id = $this->department->insert('department', $data);
                if ($insert_id) {                    
                    create_log('Has been created a department :'.$data['name']);
                    
                    success($this->lang->line('insert_success'));
                    redirect('hrm/department/index/'.$data['school_id']);
                } else {
        ...
        $this->data['department'] = $this->department->get_department_list();      
        
        $condition = array();
        $condition['status'] = 1;        
        if($this->session->userdata('role_id') != SUPER_ADMIN){            
            $condition['school_id'] = $this->session->userdata('school_id');        
            $this->data['teachers'] = $this->department->get_list('department', $condition, '','', '', 'id', 'ASC');
        }        
        $this->data['schools'] = $this->schools;
    ...
    public function edit($id = null) {       
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                $data = $this->_get_posted_class_data();
                $updated = $this->department->update('department', $data, array('id' => $this->input->post('id')));
                ...
                if ($updated) {
                 ...
                 error($this->lang->line('update_failed'));
                $this->data['department'] = $this->department->get_single('department', array('id' => $this->input->post('id')));
            }
        }
        
        if ($id) {
            $this->data['department'] = $this->department->get_single('department', array('id' => $id));
        ...
        $this->data['filter_school_id'] = $this->data['department']->school_id;
        $this->data['schools'] = $this->schools;
   ...
   {             
      ...
      {   
          $name = $this->department->duplicate_check($this->input->post('school_id'), $this->input->post('name')); 
          if($name){
         ...
         $name = $this->department->duplicate_check($this->input->post('school_id'), $this->input->post('name'), $this->input->post('id')); 
          if($name){
                $this->form_validation->set_message('name', $this->lang->line('already_exist'));         
                return FALSE;
          } else {
    ...
    public function delete($id = null) {
        ...
        $department = $this->department->get_single('department', array('id' => $id));
        
        if ($this->department->delete('department', array('id' => $id))) {
        ...
        redirect('hrm/department/index/'.$department->school_id);
    }
    ...
    private function __create_default_section($insert_id){
        ...
        $data['created_by'] = logged_in_user_id();
        $data['status']     = 1; 
        $this->department->insert('sections', $data);
    }
